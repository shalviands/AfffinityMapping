...
1. RETURN ONLY A VALID JSON OBJECT with a "cards" key containing an array of insight objects. No preamble. No postamble. No explanation.
2. NO MARKDOWN. NO CODE FENCES (\`\`\`). NO FENCES (---).
3. YOUR RESPONSE MUST START WITH \`{\` AND END WITH \`}\`.
4. IF THE TRANSCRIPT IS INCOMPLETE, RETURN A PARTIAL BUT VALID JSON OBJECT.
5. IF NO INSIGHTS ARE DETECTED, RETURN \`{"cards": []}\`.
...
— Add any text, notes, or apologies outside the JSON structure
...
export const buildExtractionPrompt = (transcript: string, researchQuestion: string, sector: string, stage: string) => {
...
Return ONLY a JSON object with a "cards" key. No markdown. No code fences. No explanation.
...
export const buildClusteringPrompt = (cards: any[], researchQuestion: string, sector: string, stage: string) => {
...
export const buildHMWPrompt = (clusterName: string, synthesis: string, researchQuestion: string, sector: string) => {
...
export const buildJTBDPrompt = (clusters: any[], sector: string) => {
  const clusterData = JSON.stringify(
    clusters.map(c => ({
      ...
      negativeCards: c.cards.filter((card: any) => card.sentiment === 'negative').length,
      topCards: c.cards.slice(0, 5).map((card: any) => card.insight),
    })),
    null,
    2
  );
...
export const buildLeanCanvasPrompt = (clusters: any[], sector: string, stage: string) => {
  const clusterData = JSON.stringify(
    clusters.map(c => ({
      name: c.name,
      synthesis: c.synthesis,
      ...
      highFrequencyInsights: c.cards
        .filter((card: any) => card.freq > 1)
        .map((card: any) => `${card.insight} (mentioned ${card.freq} times)`),
      topInsights: c.cards.slice(0, 5).map((card: any) => card.insight),
      speakersRepresented: [...new Set(c.cards.map((card: any) => card.speaker))].length,
    })),
    null,
    2
  );
