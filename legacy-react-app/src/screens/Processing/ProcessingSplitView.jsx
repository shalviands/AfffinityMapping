...
export default function ProcessingSplitView() {
  ...
  useEffect(() => {
    ...
    function chunkTranscript(text, size = 6000) {
        if (!text) return ["Speaker 1: Please wait, no transcript detected yet."];
        const chunks = [];
        let index = 0;
    ...
    async function processTranscript() {
        ...
        try {
           ...
           for (const [i, chunk] of transcriptChunks.entries()) {
               ...
               // Robust extraction: Handle {cards: [...]}, {insights: [...]}, or direct [...]
               const parsed = res.data;
               const cardsToProcess = Array.isArray(parsed) 
                   ? parsed 
                   : (parsed.cards || parsed.insights || []);

               if (!Array.isArray(cardsToProcess)) {
                   console.warn(`[Processing] No valid card array found in AI response for chunk ${i+1}`);
                   continue;
               }

               const populated = cardsToProcess.map((c, idx) => 
                   normalizeCard(c, `${i}-${idx}`, session.stakeholderName, id)
               );
           ...
           // Heuristic Fallback: If AI returned 0 results after all chunks, extract points line-by-line
           if (allExtracted.length === 0 && transcript) {
               console.warn("[Processing] AI extraction yielded no results. Triggering heuristic fallback.");
               const lines = transcript.split('\n')
                   .map(l => l.trim())
                   .filter(l => l.length > 15 && !l.toLowerCase().includes("no bhashini"));
               
               allExtracted = lines.map((l, i) => normalizeCard({
                   insight: l.length > 100 ? l.substring(0, 100) + '...' : l,
                   quote: l,
                   sentiment: 'neutral',
                   confidence: 'low',
                   theme: 'Raw Signal'
               }, `fallback-${i}`, session.stakeholderName, id));
               
               if (mounted) {
                   setProcessingError("AI Extraction restricted. Switched to 'Raw Signal' mode (using transcript lines).");
               }
           }

           if (mounted) {
               setAiMetadata({
                   model: lastModel || (allExtracted.length > 0 ? 'Heuristic Fallback' : 'None'),
               ...
               if (allExtracted.length > 0 && !processingError?.includes("Extraction restricted")) {
      ...
      <div className="flex-1 p-6 flex flex-col bg-slate-50 relative overflow-hidden">
         ...
         {isProcessing ? (
            ...
            <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto pb-8 pr-2 flex-1">
